...
function mapCampaign(row: any, memberRows: any[] = []): Campaign {
  ...
  return {
    ...
    nextSessionId: row.next_session_id ?? undefined,
...
function toCampaignRow(campaign: Campaign) {
  return {
    ...
    system_profile: campaign.systemProfile ?? {},
    public_notes: campaign.publicNotes ?? null,
    gm_notes: campaign.gmNotes ?? null,
    next_session_id: campaign.nextSessionId ?? null,
...
function mapCampaignSession(row: any): CampaignSession {
  return {
    ...
    liveNotes: row.live_notes ?? undefined,
    recap: row.recap ?? undefined,
    openQuestions: row.open_questions ?? [],
    nextHooks: row.next_hooks ?? [],
...
function toCampaignSessionRow(session: CampaignSession) {
  return {
    ...
    arc_id: session.arcId ?? null,
    chapter_id: session.chapterId ?? null,
    name: session.name,
    scheduled_at: session.scheduledAt ?? null,
    status: session.status ?? "planned",
    objective: session.objective ?? null,
    preparation_notes: session.preparationNotes ?? null,
    notes: session.notes ?? null,
    live_notes: session.liveNotes ?? null,
    recap: session.recap ?? null,
    open_questions: session.openQuestions ?? [],
    next_hooks: session.nextHooks ?? [],
    active_scene_id: session.activeSceneId ?? null,
    scene_ids: session.sceneIds ?? [],
